...
public:
    ...
    */
    ...
    static bool AllowFree(double dPriority)
    {
        ...
        // need a fee.
        return dPriority > LARGE_LOW_FEE;
    }
...
public:
    ...
    unsigned int nTime;
    unsigned int nBits;  // pNut: prime chain target, see prime.cpp
    unsigned int nNonce;

    // pNut: proof-of-work certificate
    ...
    // pNut: header hash does not include prime certificate
    uint256 GetHeaderHash() const
    {
        return Hash(BEGIN(nVersion), END(nNonce));
    }

    // pNut: block hash includes prime certificate
    uint256 GetHash() const
    {
...
public:
    ...
    unsigned int nPrimeChainType;   // pnut: chain type (memory-only)
    unsigned int nPrimeChainLength; // pnut: chain length (memory-only)
    ...
    bool ReadFromDisk(const CDiskBlockPos &pos)
    {
        ...
        // pNut: no proof-of-work check here unlike bitcoin
        // Check the header
        return true;
    }
...
public:
    ...
    uint256 nChainWork;
    unsigned int nWorkTransition; // pnut: work transition ratio (memory-only)

    unsigned int nPrimeChainType;   // pnut: chain type
    unsigned int nPrimeChainLength; // pnut: chain length
    int64 nMoneySupply;             // pnut: money supply
    ...
    {
        // pNut: disabled proof-of-work check for loading block index
...
public:
    uint256 hashPrev;
    uint256 hashBlock; // pnut: persist block hash as well
...
public:
    ...
    void Set(const CBlockIndex* pindex)
    {
        ...
        vHave.push_back(GENESIS_BLOCK_HASH);
    }

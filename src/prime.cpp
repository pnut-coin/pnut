...
unsigned int nTargetInitialLength = 5; // initial chain length target
unsigned int nTargetMinLength = 4;     // minimum chain length target
...
// pNut mint rate is determined by target
//   mint = 999 / (target length ** 2)
// Inflation is controlled via Moore's Law
bool TargetGetMint(unsigned int nBits, uint64& nMint)
{
    nMint = 0;
    static uint64 nMintLimit = 249llu * COIN;
...
bool TargetGetNext(unsigned int nBits, int64 DIFFICULTY_MODIFIER, int64 TARGET_TIME_SPACING, int64 nActualSpacing, unsigned int& nBitsNext)
{
    nBitsNext = nBits;
    ...
    bnFractionalDifficulty *= ((DIFFICULTY_MODIFIER + 1) * TARGET_TIME_SPACING);
    bnFractionalDifficulty /= ((DIFFICULTY_MODIFIER - 1) * TARGET_TIME_SPACING + nActualSpacing + nActualSpacing);
